...
void LcdPrintString(char *s, unsigned char positionX,  unsigned char positionY);
/**********************************************************************
 * ---------------------- LCD global variables ------------------------
 **********************************************************************/
EventMaskType LCDevent;
unsigned char LCDchar[LINE][COLUMN];
extern unsigned char mapData[4][16];
unsigned char firstRow[17];
unsigned char secondRow[17];
unsigned char thirdRow[17];
unsigned char fourthRow[17];
...
TASK(LCD)
{
    /* Following three lines are for 
     making necessary initializations for LCD
     */
    ADCON1 = 0x8E;
    ADCON0 = 0x00;
    InitLCD();
    ...
    fourthRow[16] = 0;
    /*
     * This alarm wakes LCD event in every 350ms
     * In this way, our LCD can refresh with row datas of map
     * and LCD state
     */
    SetRelAlarm(LCD_ALARM_MAIN, 100, 350);
    while(1)
    {
        // Wait LCD_EVENT to fire
        WaitEvent(LCD_EVENT);
        ClearEvent(LCD_EVENT);
        //Below loop are for initializing row datas of map.
        for(counter=0 ; counter<16 ; counter++){
            if(mapData[0][counter]==0){
                firstRow[counter] = 'X';
            }
            else if(mapData[0][counter]==1){
                firstRow[counter] = ' ';
            }
            else{
                firstRow[counter] = 0XFF;
            }

            if(mapData[1][counter]==0){
                secondRow[counter] = 'X';
            }
            else if(mapData[1][counter]==1){
                secondRow[counter] = ' ';
            }
            else{
                secondRow[counter] = 0XFF;
            }
            
            if(mapData[2][counter]==0){
                thirdRow[counter] = 'X';
            }
            else if(mapData[2][counter]==1){
                thirdRow[counter]=' ';
            }
            else{            
                thirdRow[counter]=0XFF;
            }

            if(mapData[3][counter]==0){
                fourthRow[counter]='X';
            }
            else if(mapData[3][counter]==1){
                fourthRow[counter]=' ';
            }
            else{
                fourthRow[counter]=0XFF;
            }
        }
        
        //if current LCD state is zero, print above 2 rows of map, else below 2 rows. 
        if(state == 0){
            LcdPrintString(firstRow, 0 ,0);
            LcdPrintString(secondRow, 0 ,1);
        }
        else{
            LcdPrintString(thirdRow, 0 ,0);
            LcdPrintString(fourthRow, 0 ,1);
        }
